...
void hit(state_model* current_model, const koef_of_model* koef_model, const surface* current_surface) {
	 Vector2 guide_surface; //Surface vector guide
...
int touch_test(state_model* current_model, const  koef_of_model* koef_model, const surface* current_surface, double *current_surface_height) {
	//Find the height of the surface under the body
	const auto cur_surface_height = tan(current_surface->angle) * current_model->wheel.Coord.x + current_surface->start_y - current_surface->start_x * tan(current_surface->angle);
	if (current_surface_height != nullptr)
...
int speed_into_surface(const  state_model* current_model, const  surface* current_surface) {//Determine the speed is directed into the surface of it
	if (current_model->wheel.Velocity.y / norm(current_model->wheel.Velocity) > sin(current_surface->angle)) return 0;//out
	else return 1; //in
}

void hit_theend_step( state_model* const current_model, const  koef_of_model* koef_model, const surface* current_surface, const bool contact) {
...
Vector2 center_mass(const state_model *current_model, const koef_of_model *koef_model) {
	//center = 1/M * SUM(m_i*r_i); i = 1...N
	auto center = current_model->body.Coord * koef_model->body.mass + current_model->wheel.Coord * koef_model->wheel.mass;
...
double deformation_suspension(const Vector2 body, const Vector2 wheel, const koef_of_model *const koef_model) {
	auto distance = norm(body, wheel);
	if (body.y < wheel.y) distance = -distance;
	const auto deformation = koef_model->wheel.position - distance;//We assume the deformation of the spring
	return deformation;
}