...
extern "C" __declspec(dllexport) state_model*  initialization_koef_model(const Vector2 body_coord, const Vector2 *const wheel_coord) {
	...
	current_model.body.state_body.velocity.set0();
	current_model.body.state_body.acceleration.set0();
	current_model.body.state_body.force.set0();
	current_model.body.state_body.pitch = 0;
	...
	for (auto i = 0; i <current_model.number_wheel; ++i) {
		...
		current_model.wheel[i].state_wheel.velocity.set0();
		current_model.wheel[i].state_wheel.acceleration.set0();
		current_model.wheel[i].state_wheel.force.set0();
		current_model.wheel[i].state_wheel.pitch = 0;
...
extern "C" __declspec(dllexport) void destroy_model (state_model *current_model){
	if (current_model->wheel != nullptr) delete[] current_model->wheel;
}
