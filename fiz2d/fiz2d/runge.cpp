...
int next_step_n(FILE* log,  state_model* current_model, const  koef_of_model* koef_model, const  surface* current_surface, const double force, double* time_left, double all_time_step) {
	fprintf(log, "! ! !runge_N\n");
	...
	while (true) {
		const auto Approximate_movement = current_model->wheel.Velocity * step_time + current_model->wheel.Acceleration * step_time*step_time / 2; //Approximate movement calculation in lineral model
		...
		// solve fo step h
		const auto Rcoord = current_model->wheel.Coord + solve_koef_coord(&K);
		const auto Rvelocity = current_model->wheel.Velocity + solve_koef_velocity(&K);

		runge_K Km;
		runge_koef(current_model, current_model->wheel.Velocity, current_model->wheel.Acceleration, koef_model, current_surface, force, step_time / 2, &Km); //Calculation koef for step h/2
		
		//solve for step h/2
		auto Rcoord_half = current_model->wheel.Coord + solve_koef_coord(&Km);
		auto Rvelocity_half = current_model->wheel.Velocity + solve_koef_velocity(&Km);

		const auto solve_Acceleration = func_solve_acc_wheel(current_model, &Rvelocity_half, koef_model, force, current_surface, false); //Calculation of a new acceleration
		runge_koef(current_model, Rvelocity_half, solve_Acceleration, koef_model, current_surface, force, step_time / 2, &Km); //Calculation koef for time+h and step h/2

		//solve for time+h and step h/2
		Rcoord_half += solve_koef_coord(&Km);
		Rvelocity_half += solve_koef_velocity(&Km);

		auto err = err_runge(Rcoord, Rcoord_half); //find max err
		const auto tmp_err = err_runge(Rvelocity, Rvelocity_half);
		...
		if (err < eps || step_time < all_time_step / max_num_step) { //Error checking
			...
			*time_left -= step_time;
			auto tmp = solve_body(log, current_model, koef_model, step_time);
...
int solve_body(FILE* log, state_model *current_model, const koef_of_model *koef_model, double all_time_step) {
	...
	auto step_time = all_time_step;
	auto time_left = all_time_step;
	...
	while (time_left > 0) {
		...
		// solve fo step h
		const auto Rcoord = current_model->body.Coord + solve_koef_coord(&K);
		const auto Rvelocity = current_model->body.Velocity + solve_koef_velocity(&K);

		runge_K Km;
		runge_koef_body(current_model->body.Velocity, current_model->body.Acceleration, koef_model, step_time / 2, &K);//Calculation koef for step 
		//solve for step h/2
		auto Rcoord_half = current_model->body.Coord + solve_koef_coord(&Km);
		auto Rvelocity_half = current_model->body.Velocity + solve_koef_velocity(&Km);

		//next half step
		const Vector2 half_acceleration = func_solve_acc_body(Rcoord_half, current_model->wheel.Coord, koef_model, nullptr); //solv acceleration
		runge_koef_body(Rvelocity_half, half_acceleration, koef_model, step_time / 2, &K);//Calculation koef for step
		Rcoord_half += solve_koef_coord(&Km);
		Rvelocity_half += solve_koef_velocity(&Km);

		auto err = err_runge_y(Rcoord, Rcoord_half); //find max err
		const auto tmp_err = err_runge_y(Rvelocity, Rvelocity_half);
		if (err < tmp_err) err = tmp_err;
