...
extern "C" __declspec(dllexport) int solve_step (state_model* current_model, koef_of_model* koef_model, surface *current_surface, double* step_time, double* force) {
	...
	while (step_time > 0) {
		...
		else
		{
			//we are flying
			double step = 0.0;
			touch_earth touch_state = find_earth(current_model, koef_model, current_surface, &step);
			switch (touch_state)
			{
			case touch_earth::normal: //Touching a surface within a known surface
			{
				if (step > *step_time)
					step = *step_time;
			...
			case touch_earth::outside_surface: //Touching a surface outside a known surface
			{
			...
			case touch_earth::fall_down_texture: //This should never happen //We failed
				current_model->Coord.y += 50;
			}
		}
