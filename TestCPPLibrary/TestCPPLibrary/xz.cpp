...
double find_earth(state_model* current_model, koef_of_model* koef_model, surface* current_surface) {
	...
	double D = b * b - 2 * koef_model->gravity * (tan(current_surface->angle) * current_model->Coord.x + koef_model->radius - current_model->Coord.y); //Discriminant
...
int touch_test(state_model* current_model, surface* current_surface) {
	double ñurrent_surface_height = tan(current_surface->angle) * current_model->Coord.x + current_surface->start_y - current_surface->start_x * tan(current_surface->angle);
