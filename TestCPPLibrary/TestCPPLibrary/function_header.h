...
inline void next_step_no_N(struct state_model*, const struct koef_of_model*, const double time);

//runge_func.cpp
inline struct Vector2 func_solve_acceleration(const struct Vector2* Velocity, const struct koef_of_model*, const double force, const struct surface*);
inline struct Vector2 ñonversion_inertial_coordinate_system(const double val, const struct surface* current_surface);
inline void runge_koef(const struct Vector2* solve_velocity, const Vector2* solve_acceleration, const struct koef_of_model*, const struct surface*, const double force, const double step, struct runge_K* K); // Attention * &
inline struct Vector2 solve_koef_coord(const struct runge_K*);
inline struct Vector2 solve_koef_velocity(const struct runge_K*);
